...
namespace unitMySql {
	...
	public class setupDb {
		...
		public static void close(int handler) {
			...
			param["driver"] = getDriver();
			EdgeCompiler ec = new EdgeCompiler();
			Task<object> resClose = ec.CompileFunc(param);
	...
	public class Environment {
		...
		public void testEnvironmentVersion() {
			...
			Assert.That(frameworkDescription, Is.EqualTo(".NET 7.0.4"), "Environment check");
		}
		...
		public void testDllVersion() {
			...
			foreach (Assembly A in currAss) {
				sb.AppendLine(A.GetName().Name.PadLeft(40) + ":" + A.GetName().Version.ToString());
				versions[A.GetName().Name] = A.GetName().Version.ToString();
			}

			

			string frameworkDescription = System.Runtime.InteropServices.RuntimeInformation.FrameworkDescription;
			string s = frameworkDescription;
			s += sb.ToString();
			Console.WriteLine(s);
			Assert.That(1, Is.EqualTo(1), "Environment list");
		}
	}
	...
	public class MySql {
		...
		public void openConnection() {
			...
			var res = ec.CompileFunc(param);			
			Task.WaitAll(res);
			Assert.That(res.Status, Is.EqualTo(TaskStatus.RanToCompletion), "Open executed");
		...
		public void closeConnection() {
			...
			try {
				var resClose = ec.CompileFunc(param);				
				Task.WaitAll(resClose);
		...
		public void setupScriptShouldExist() {
			...
			string path = Path.Combine(
						Directory.GetCurrentDirectory(), "setup.sql");
